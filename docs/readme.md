...
At the moment, the project uses daisyUI v4, which was the latest version at the start of Sprint 1. During Sprint 4 (on 28 Feb, 2025), daisyUI v5 (<https://daisyui.com/docs/v5/>) was released, which includes a large number of improvements to the components, their accessibility, and the bundle size of the library. This was a major major update to the library, with a number of breaking changes, including to the way daisyUI is imported, and the naming scheme for its CSS variables. This means some work will be required to update the project to use daisyUI v5, but it will bring a number of user-facing benefits, so it would be one of the first things I would do if I were to continue developing the project. There is a migration guide available at <https://daisyui.com/docs/upgrade/>.
